...
import { COMPTE_DEVISE, COMPTE_EPARGNE, COMPTE_COURANT } from '../../app/models/TypeCompte';
  ...
  /* it("Doit retourner 401 si l'email/user existe",
    ...
    Chai.request(testServer)
      ...
      .end(function(err,res){
        ...
        res.body.should.have.property('code_err').equals('U01')
        done()
      })    
  });
...
/* describe('Création d\'un autre compte bancaire', function() {
  ...
  it("Doit retourner 400 si le code monnaie est erroné", 
    function(done){
          ...
          .end(function(err,res){
            err.should.have.status(400)
            res.body.should.have.property('code_err').equals('C02')
            console.log(res.body.msg_err)
            done()
          })
  });
  ...
  it("Doit retourner 400 si le type du compte à créer est erroné",
    function(done){
          ...
          .end(function(err,res){
            err.should.have.status(400)
            res.body.should.have.property('code_err').equals('C01')
            console.log(res.body.msg_err)
            done()
          })  
  });
  ...
  it("Doit retourner 400 si le type du compte à créer est un compte courant",
    function(done){
        ...
        .end(function(err,res){
          err.should.have.status(400)
          res.body.should.have.property('code_err').equals('C03')
          console.log(res.body.msg_err)
          done()
        }) 
  });

  it("Doit retourner 200 si un compte épargne est créé avec succès",
    function(done){
    Compte.destroy({
      where:{
        id_user:idClient,
  ...
  it("Doit retourner 200 si un compte devise est créé avec succès",
    function(done){
    Compte.destroy({
      where:{
        id_user:idClient,
  ...
  it("Doit retourner 400 si le type du compte à créer existe déjà (Epargne)",
    ...
    .end(function(err,res){
      err.should.have.status(400)
      res.body.should.have.property('code_err').equals('C04')
      console.log(res.body.msg_err)
      done()
    }) 
  });
  ...
  it("Doit retourner 400 si le code monnaie ne correspond pas au type du compte à créer",
    ...
    .send({
      user: idClient,
      type_compte:COMPTE_DEVISE, //COMPTE_EPARGNE
      monnaie:"DZD"     //EUR
    })
    .end(function(err,res){
      err.should.have.status(400)
      res.body.should.have.property('code_err').equals('C05')
      console.log(res.body.msg_err)
      done()
    }) 
  });

  it("Doit retourner 200 si un compte devise (Devise USD) est créé avec succès",
    function(done){
    Compte.destroy({
      where:{
        id_user:idClient,
